...
@implementation LCPNetWorkERROR
...
+(NSError *) httpError:(NSURLSessionDataTask*)task withUserinfo:(NSDictionary*) _userinfo{
    ...
    if(task){
        ...
        if (jsonObject && [jsonObject isKindOfClass:[NSDictionary class]]) {
            ...
            NSLog(@"error[msg]= == %@",jsonObject[@"msg"]);
        }
...
+(void)dealErrorInfo:(NSError *)error Task:(NSURLSessionDataTask *)task CompeleteBlcok:(void (^)(NSError *))completeBlcok{
    ...
    NSLog(@"---%@",[[newError userInfo] objectForKey:@"NSLocalizedDescription"]);
    NSLog(@"____****____%@",[[newError userInfo] objectForKey:@"NSErrorFailingURLKey"]);
